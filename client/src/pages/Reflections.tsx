...
export default function Reflections() {
  const { state, saveReflection, updateMonthlyIntention, updateQuarterlyGoal } = useApp();
  ...
  // Keep quarterly goal in sync with context
  useEffect(() => { setQuarterlyGoalText(state.quarterlyGoal?.text || ''); }, [state.quarterlyGoal?.text]);
      ...
      <div className="mx-4 mb-4 p-4 rounded-2xl border border-[var(--border)] bg-white">
            ...
            <button
              onClick={() => {
                if (tab === 'monthly') {
                  ...
                  if (editingQuarterlyGoal) { updateQuarterlyGoal({ text: quarterlyGoalText }); toast.success('Quarterly goal saved!'); }
                  setEditingQuarterlyGoal(e => !e);
                }
              }}
              className="text-xs text-[var(--sky)] font-semibold shrink-0 flex items-center gap-1"
            >
              {(tab === 'monthly' ? editingIntention : editingQuarterlyGoal) ? (
        ...
        {tab === 'monthly' ? (
          ...
          editingQuarterlyGoal ? (
            ...
            <p className="text-sm text-[var(--foreground)] leading-relaxed font-['Playfair_Display'] italic">
              {quarterlyGoalText || <span className="text-[var(--muted-foreground)]">Tap Edit to set your quarterly focus goal.</span>}
            </p>
          )
        ) : (
          <p className="text-sm text-[var(--foreground)] leading-relaxed font-['Playfair_Display'] italic">
